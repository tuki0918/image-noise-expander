...
def expand_with_noise_and_crop_gradio(
    img, mode="outside", shift_pct=30, direction="bottom"
):
    img = img.convert("RGB")
    w, h = img.size
    ...
    if mode == "inside":
        ...
        if direction == "bottom":
            noise = np.random.normal(
                loc=127, scale=255 * noise_strength, size=(shift_h, w, 3)
            )
            ...
            noise = np.random.normal(
                loc=127, scale=255 * noise_strength, size=(h, shift_w, 3)
            )
            noise = np.clip(noise, 0, 255).astype(np.uint8)
            ...
            raise ValueError(
                f"Invalid direction: {direction}. Must be one of top, bottom, left, right."
            )
...
def create_gradio_interface():
    demo = gr.Interface(
        ...
        inputs=[
            ...
            gr.Radio(
                ["top", "bottom", "left", "right"], value="bottom", label="Direction"
            ),
        ],
        outputs=[
